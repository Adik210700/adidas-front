...
import type { HistoryEntry, HistoryList, HistoryEvent } from '@/types'
...
watch(() => props.docId, () => { if (props.modelValue) refresh() })
...
const loadMore = async () => {
  if (!props.docId || loading.value) return
  loading.value = true
  try {
    const data = await apiFetch<HistoryList>(`/api/property-documents/${props.docId}/history/?limit=${limit}&offset=${offset}`)
...
.badge.ev-status_changed {
  background: #fef3c7;
  color: #b45309;
}

.badge.ev-file_replaced {
  background: #e0f2fe;
  color: #0369a1;
}
